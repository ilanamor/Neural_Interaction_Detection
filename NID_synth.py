...
def gen_synth_data():
    ...
    with open('datasets\X.pickle','wb') as handle:
        pickle.dump(X, handle,protocol=pickle.HIGHEST_PROTOCOL)

    with open('datasets\Y.pickle','wb') as handle:
        pickle.dump(Y, handle,protocol=pickle.HIGHEST_PROTOCOL)

    np_array = np.concatenate((X, Y), axis=1)
    df = pd.DataFrame(np_array)
    pd.DataFrame(X).to_csv("datasets\X.csv", header=False, index=False)
    pd.DataFrame(Y).to_csv("datasets\Y.csv", header=False, index=False)
